...
var Broker =function(url) {
  ...
  this.source= function(param,NS,Name,CallBack){
    console.log(this.server+this.webapp +"source/"+ param +"?NameSpace="+NS+"&Name="+Name )
    http.get(this.server+this.webapp +"source/"+ param +"?NameSpace="+NS+"&Name="+Name , function(response){
      CallBack(response);
    });


  /*client.get( this.server+this.webapp +"source/"+ param +"?NameSpace="+NS+"&Name="+Name ,{}, function(data, response){
    console.log(data)
    CallBack(JSON.parse(data));
  })*/
